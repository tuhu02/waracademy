...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Black+Ops+One&family=Poppins:wght@400;600&display=swap"
        rel="stylesheet">
...
<body class="bg-gradient-to-b from-[#0f1b2e] via-[#243b55] to-[#3b5875] min-h-screen text-white font-poppins">
    ...
    <img src="/images/war.png" alt="Logo" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) scale(1.5);
    ...
    <div class="relative z-10 p-6 max-w-4xl mx-auto">
        ...
        @if(empty($questions) || count($questions) === 0)
            <div class="bg-black/20 backdrop-blur-lg border border-red-400/30 rounded-xl shadow-2xl p-8 text-center">
                ...
                <p class="text-gray-300 text-lg mb-6">Turnamen ini belum memiliki soal atau soal belum memiliki pilihan
                    jawaban. Silakan hubungi guru untuk menambahkan soal.</p>
        ...
        @else
            <script>
                // Pass data to global scope for Alpine.js
                window.tournamentData = {
                    duration: {{ $turnamen->durasi_pengerjaan }},
                    questions: @json($questions),
                    submitUrl: '{{ route('tournament.submit', ['id' => $turnamen->id_turnamen]) }}'
                };
            </script>
            <div x-data="quizController(
                ...
                <div x-show="!isFinished" x-cloak>
                    <div
                        ...
                        <div class="flex justify-between items-center p-4 bg-black/30 border-b border-blue-400/30">
                            <div class="text-lg">
                                Soal <strong class="text-cyan-300" x-text="currentQuestionIndex + 1"></strong> / <span
                        ...
                        <div class="p-6 md:p-8 min-h-[300px]">
                            ...
                            <div class="space-y-3">
                                <template x-for="option in currentQuestion.options" :key="option.id_jawaban">
                                    <button @click="selectAnswer(currentQuestion.id, option.id_jawaban)"
                        ...
                        <div class="flex justify-between items-center p-4 bg-black/30 border-t border-blue-400/30">
                            ...
                            <template x-if="currentQuestionIndex === questions.length - 1">
                                <button @click="submitQuiz"
                                    class="btn-nav bg-green-600 border-green-400 hover:bg-green-500">
                ...
                <div x-show="isFinished" x-cloak>
                    <div
                        class="bg-black/20 backdrop-blur-lg border border-blue-400/30 rounded-xl shadow-2xl p-8 text-center">
                        ...
                        <svg class="animate-spin h-12 w-12 text-green-400 mx-auto mb-6" xmlns="http://www.w3.org/2000/svg"
    ...
    <script>
        function quizController(durationInMinutes, questionsData, submitUrl) {
            // Pastikan questionsData adalah array
            ...
            return {
                ...
                timeRemaining: (durationInMinutes || 45) * 60,
                timerText: `${durationInMinutes || 45}:00`,
                timerInterval: null,
                isFinished: false,
                submitUrl: submitUrl || '',
                ...
                init() {
                    // Pastikan questions ada dan tidak kosong sebelum start timer
                    if (!this.questions || this.questions.length === 0) {
                        return;
                    }
