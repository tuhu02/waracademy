...
<head>
    ...
    <style>
        body {
            ...
            background-image: url('/images/tournament.png'); /* Pastikan path ini benar */
        ...
        .team-slot {
            width: 80px; /* Lebar slot */
            height: 100px; /* Tinggi slot + nama */
            display: flex;
...
<body class="relative font-poppins min-h-screen flex flex-col justify-between p-4 md:p-6 text-white overflow-hidden select-none">
    ...
    @if($turnamen->mode === 'tim')
        ...
        <div x-data="teamLobbyController(
            ...
            <header class="relative w-full text-center z-10 mb-6">
                <h1 class="font-blackops text-2xl md:text-4xl font-extrabold bg-gradient-to-b from-[#dce6f2] via-[#a3d3fa] to-[#6aa8fa] bg-clip-text text-transparent drop-shadow-[0_0_15px_rgba(70,150,255,0.6)] mb-2">
                ...
                <div class="mt-2 text-sm text-gray-200 bg-black/20 backdrop-blur-sm inline-block px-3 py-1 rounded-full border border-cyan-400/50">
                    Mode: <strong class="font-semibold text-white">Tim</strong>
                    <span class="mx-2">|</span>
                    Maks. Anggota: <strong class="font-semibold text-white">{{ $turnamen->max_team_members }} orang</strong>
                </div>
            </header>
                    ...
                    <template x-for="team in teams" :key="team.id">
                        <div class="team-box">
                            ...
                            <div class="flex flex-wrap gap-3 justify-center">
                                <template x-for="(member, index) in team.members" :key="index">
                                    <div class="team-slot">
                                        <div @click="clickSlot(team.id, index)"
                                             :class="{
                                                'slot-box': true,
                                            ...
                                            <template x-if="member">
                                                <img :src="member.avatar_url || 'https://ui-avatars.com/api/?name=' + member.username + '&background=0b2239&color=38bdf8'" 
                                                     :alt="member.username" 
                                                     class="slot-avatar">
                                            </template>
        ...
        <div x-data="soloLobbyController(
            ...
            <main class="flex-1 relative flex flex-col justify-start items-center z-10 overflow-y-auto">
                ...
                <div class="text-center mb-6">
                    <p class="text-gray-300 mb-4" id="participantCount" x-text="`${participants.length} / {{ $turnamen->max_peserta }} Peserta Telah Bergabung`"></p>
                    
                    <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-4 justify-center max-w-2xl mx-auto" id="participantsContainer">
        ...
        function teamLobbyController(roomCode, userId, userName, userAvatar, initialTeams) {
            return {
                ...
                async clickSlot(teamId, slotIndex) {
                    ...
                    // Optimistic UI update (langsung pindah di layar)
                    this.leaveCurrentSlot(); // Hapus dari slot lama
                    targetTeam.members[slotIndex] = { id: this.myId, username: this.myName, avatar_url: this.myAvatar };

                    // Kirim ke server
                    const success = await this.sendMoveRequest(teamId, slotIndex);
                    if (!success) {
                        // Jika gagal (misal slot diambil orang lain), 
                        // polling berikutnya akan otomatis memperbaiki UI.
                        // Untuk sekarang, kita kembalikan user ke 'null'
                        this.leaveCurrentSlot();
                    }
                    this.isMoving = false;
                },
